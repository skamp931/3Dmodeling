...
def create_elliptical_cylinder_mesh(center_pos, radius_x, radius_y, height, n_segments=32):
    ...
    x_c = radius_x * np.cos(theta); y_c = radius_y * np.sin(theta)
    verts = []
...
def create_elliptical_frustum_mesh(center_pos, bottom_radius_x, bottom_radius_y, top_radius_x, top_radius_y, height, n_segments=32):
    """通常の楕円円錐台のメッシュを生成する"""
    theta = np.linspace(0, 2 * np.pi, n_segments, endpoint=False)
...
def clip_mesh_by_plane(vertices, faces, plane_func):
    ...
    for face in faces:
        ...
        # 面が平面と交差する場合
        # (簡単のため、ここでは交差する面の分割は省略し、頂点位置に基づいて大まかに分類)
        # より正確な表示には、面の分割アルゴリズムが必要
...
def get_default_pillars_config():
    dist = 7.0 / 2.0; rx_ratio = 1.0; ry_ratio = 3.0
    config = {
        'A': {'pos':[-dist, dist],'foundation_h':1.0,'foundation_r_bottom_x':3.0*rx_ratio,'foundation_r_bottom_y':3.0*ry_ratio,'foundation_r_top_x':2.5*rx_ratio,'foundation_r_top_y':2.5*ry_ratio,'base_cyl_r_x':2.5*rx_ratio,'base_cyl_r_y':2.5*ry_ratio,'base_cyl_h':1.5,'main_cyl_r_x':0.5*rx_ratio,'main_cyl_r_y':0.5*ry_ratio,'main_cyl_h':6.0},
        'B': {'pos':[dist, dist],'foundation_h':1.0,'foundation_r_bottom_x':3.0*rx_ratio,'foundation_r_bottom_y':3.0*ry_ratio,'foundation_r_top_x':2.5*rx_ratio,'foundation_r_top_y':2.5*ry_ratio,'base_cyl_r_x':2.5*rx_ratio,'base_cyl_r_y':2.5*ry_ratio,'base_cyl_h':1.5,'main_cyl_r_x':0.5*rx_ratio,'main_cyl_r_y':0.5*ry_ratio,'main_cyl_h':6.0},
        'C': {'pos':[dist, -dist],'foundation_h':1.0,'foundation_r_bottom_x':3.0*rx_ratio,'foundation_r_bottom_y':3.0*ry_ratio,'foundation_r_top_x':2.5*rx_ratio,'foundation_r_top_y':2.5*ry_ratio,'base_cyl_r_x':2.5*rx_ratio,'base_cyl_r_y':2.5*ry_ratio,'base_cyl_h':1.5,'main_cyl_r_x':0.5*rx_ratio,'main_cyl_r_y':0.5*ry_ratio,'main_cyl_h':6.0},
        'D': {'pos':[-dist, -dist],'foundation_h':1.0,'foundation_r_bottom_x':3.0*rx_ratio,'foundation_r_bottom_y':3.0*ry_ratio,'foundation_r_top_x':2.5*rx_ratio,'foundation_r_top_y':2.5*ry_ratio,'base_cyl_r_x':2.5*rx_ratio,'base_cyl_r_y':2.5*ry_ratio,'base_cyl_h':1.5,'main_cyl_r_x':0.5*rx_ratio,'main_cyl_r_y':0.5*ry_ratio,'main_cyl_h':6.0},
    }
    return config
...
for pillar_id, config in pillars_config.items():
    ...
    # --- 基礎円錐台（計算対象）---
    frustum_verts, frustum_faces = create_elliptical_frustum_mesh(foundation_pos, config['foundation_r_bottom_x'], config['foundation_r_bottom_y'], config['foundation_r_top_x'], config['foundation_r_top_y'], config['foundation_h'])
